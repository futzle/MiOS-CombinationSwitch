module ("L_CombinationSwitch1Plugin_HVACSetpointHeat", package.seeall)

local MAIN

local TEMPSETPOINT_SERVICE_ID = "urn:upnp-org:serviceId:TemperatureSetpoint1_Heat"
...
function configureSelect(combinationDeviceId, index, state)
	...
	local result = MAIN.template([[
		...
		<input type="text" size="4" value="~selectedLow~" onchange='warnSave(); set_device_state(~combinationDeviceId~, "~combinationServiceId~", "~index~~INDEX_TEMPSETPOINT_VALUE_LOW~", jQuery(this).val(), 0);'>&deg; to
		<input type="text" size="4" value="~selectedHigh~" onchange='warnSave(); set_device_state(~combinationDeviceId~, "~combinationServiceId~", "~index~~INDEX_TEMPSETPOINT_VALUE_HIGH~", jQuery(this).val(), 0);'>&deg;
		]], {
...
function register(combinationDeviceId, index)
	...
	if (watchDeviceIdNum >= 0 and luup.devices[watchDeviceIdNum]) then
		...
		MAIN.debug("Watching " .. watchDeviceName .. " [" .. watchDeviceId .. "] " ..
			" Heating Setpoint: " .. watchVariableValueLow .. "-" .. watchVariableValueHigh)
